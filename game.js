...
const state={screen:'start',mission:1,selected:null,placed:{1:{},2:{},3:{},4:{},5:{},6:{}},storyIndex:0,musicOn:true,meta:{2:{index:0},4:{answers:{},checked:false},5:{index:0,order:[]}}};
...
const icons={
 ...
 play:()=>document.getElementById('icon-play').innerHTML,
 music:()=>document.getElementById('icon-music').innerHTML
};
...
5:{
 ...
 sequence:[
  {id:'wake-up',text:'I wake up early.'},
  {id:'get-up',text:'Then, I get up.'},
  {id:'get-dressed',text:'Then, I get dressed.'},
  {id:'brush-teeth',text:'Next, I brush my teeth.'},
  {id:'wash-face',text:'Then, I wash my face.'},
  {id:'breakfast',text:'Then, I have sandwiches for breakfast.'},
  {id:'swim',text:'After that, I swim in the lake.'},
  {id:'make-fire',text:'Later, we make a fire.'},
  {id:'cook',text:'Then, we cook.'},
  {id:'lunch',text:'Then, we have lunch.'},
  {id:'read-book',text:'After lunch, I read a book.'},
  {id:'play-ball',text:'Then, we play with a ball.'},
  {id:'dinner',text:'In the evening, we have dinner.'},
  {id:'board-game',text:'After dinner, we play a board game.'},
  {id:'go-bed',text:'At night, I go to bed.'}
 ],
 slots:[
  rect(28,605,170,145),rect(198,605,167,145),rect(360,605,165,145),rect(515,605,165,145),rect(675,605,160,145),
  rect(825,605,150,145),rect(965,605,160,145),rect(1115,605,155,145),rect(1260,605,160,145),rect(1410,605,175,145),
  rect(335,745,185,155),rect(515,745,175,155),rect(680,745,170,155),rect(840,745,170,155),rect(995,745,190,155)
 ]
},
6:{
...
function shell(m,overlay){
 return `<section class="game-shell" data-mission="${m}"><img class="art" src="assets/mission-${m}.webp" alt="Camp Quest Mission ${m} illustrated game field" draggable="false"><div class="layer">${missionNav()}${overlay}</div><div class="control-dock">${controlButton('music','Music on/off','music-control')}${controlButton('reset','Reset this mission')}${m>1?controlButton('back','Previous mission'):''}</div><button class="next-control" data-control="next">${m===6?'Finish':'Next'} ${icons.next()}</button><div class="toast" id="toast"></div></section>`;
}

function renderStart(){
 state.screen='start';
 app.innerHTML=`<section class="game-shell start-shell"><img class="art" src="assets/mission-1.webp" alt="Summer camping start screen" draggable="false"><div class="layer"><div class="start-overlay"></div><div class="start-card"><div class="start-kicker">Camp Quest</div><h1>It's summer!<br>The weather is nice.<br>Let's go camping.</h1><p>Pack your backpack, dress the campers, explore the weather trail, read a mini story, play the camp day game and finish with a speaking mission.</p><button class="start-button" data-control="start">${icons.play()} Start adventure</button></div></div><div class="control-dock">${controlButton('music','Music on/off','music-control')}</div></section>`;
 bindStart();
 updateMusicButtons();
}


function mission5CardsHtml(){
 const cfg=missions[5];
 const ids=state.meta[5].order.length?state.meta[5].order.slice():shuffle(cfg.sources.map(s=>s[0]));
 state.meta[5].order=ids;
 return `<div class="mission5-grid">${ids.map(id=>{
   const src=cfg.sources.find(s=>s[0]===id);
   return `<button class="source mission5-card" data-source="${id}" aria-label="${src[1]}" title="${src[1]}"><img class="mission5-card-img" src="${sourceImg(5,id)}" alt="${src[1]}"><span class="mini-check">${icons.check()}</span></button>`;
 }).join('')}</div>`;
}
...
function renderMission(m){
 ...
 if(m===5){overlay+=`<div class="mission5-wrap"><div class="mission5-panel"><div class="mission5-topline">Mission 5 • Camp Day</div><div class="mission5-title">Listen and choose the picture.</div><div class="mission5-prompt" id="m5-prompt"></div><div class="mission5-actions"><button class="mission5-listen" data-control="say5">${icons.speech()} Listen again</button><div class="mission5-progress" id="m5-progress"></div></div></div><div class="mission5-instruction">The sentence is in order. The cards are mixed. Click the correct picture.</div>${mission5CardsHtml()}</div>`;}
 if(m===6){overlay+=cfg.sources.map(s=>sourceHtml(m,s)).join('')+cfg.targets.map(t=>targetHtml(t[0],t[1],t[2],'story-target')).join('');}
 app.innerHTML=shell(m,overlay); bind(); restoreMission(m); if(new URLSearchParams(location.search).get('qa')==='1') runQA(); updateMusicButtons();
}
...
function bind(){
 ...
 shell.querySelectorAll('.source').forEach(src=>{
   if(![2,5].includes(state.mission)) src.addEventListener('pointerdown',e=>startDrag(e,src.dataset.source,false));
   src.addEventListener('click',e=>{
     if(e.detail===0||!src.dataset.dragged){
       if(state.mission===2){mission2Pick(src.dataset.source);}
       else if(state.mission===5){mission5Pick(src.dataset.source);}
...
function handleControl(c){
 if(c==='start'){resumeAudio(); if(state.musicOn) startMusic(); renderMission(1);}
 else if(c==='music'){toggleMusic();}
 else if(c==='reset'){state.placed[state.mission]={}; if(state.mission===2) state.meta[2].index=0; if(state.mission===4){state.meta[4]={answers:{},checked:false}}; if(state.mission===5){state.meta[5]={index:0,order:shuffle(missions[5].sources.map(s=>s[0]))}}; renderMission(state.mission)}
...
function ensureAudio(){ if(!audioCtx){const C=window.AudioContext||window.webkitAudioContext; if(C) audioCtx=new C();} return audioCtx; }
function resumeAudio(){ const ctx=ensureAudio(); if(ctx&&ctx.state==='suspended') ctx.resume(); }
function playTone(freq,duration=0.25,volume=0.03,type='sine',delay=0){ const ctx=ensureAudio(); if(!ctx) return; const now=ctx.currentTime+delay; const osc=ctx.createOscillator(); const gain=ctx.createGain(); osc.type=type; osc.frequency.setValueAtTime(freq,now); gain.gain.setValueAtTime(0,now); gain.gain.linearRampToValueAtTime(volume,now+0.04); gain.gain.exponentialRampToValueAtTime(0.0001,now+duration); osc.connect(gain).connect(ctx.destination); osc.start(now); osc.stop(now+duration+0.05); }
function playCorrect(){ resumeAudio(); playTone(659.25,0.18,0.05,'sine'); playTone(880,0.22,0.04,'sine',0.08); }
function playWrong(){ resumeAudio(); playTone(220,0.18,0.035,'square'); playTone(180,0.22,0.03,'square',0.08); }
function startMusic(){ if(musicTimer) return; resumeAudio(); const notes=[392,523.25,587.33,523.25]; let i=0; musicTimer=setInterval(()=>{ if(!state.musicOn) return; playTone(notes[i%notes.length],1.2,0.012,'triangle'); playTone(notes[(i+2)%notes.length]/2,1.6,0.006,'sine'); i++; },1700); }
function stopMusic(){ if(musicTimer){ clearInterval(musicTimer); musicTimer=null; } }
function toggleMusic(force){ state.musicOn = typeof force==='boolean' ? force : !state.musicOn; if(state.musicOn){ startMusic(); } else { stopMusic(); } updateMusicButtons(); toast(state.musicOn?'Music on.':'Music off.'); }
function updateMusicButtons(){ document.querySelectorAll('.music-control').forEach(btn=>{ btn.classList.toggle('off', !state.musicOn); btn.title=state.musicOn?'Music on':'Music off'; btn.setAttribute('aria-label', btn.title); }); }
function speak(text){ if(!('speechSynthesis' in window)) return; window.speechSynthesis.cancel(); const u=new SpeechSynthesisUtterance(text); u.lang='en-US'; u.rate=.88; u.pitch=1.04; window.speechSynthesis.speak(u); }
...
function mission5Current(){ return missions[5].sequence[state.meta[5].index] || null; }
function speakMission5(){ const current=mission5Current(); if(current) speak(current.text); }
function updateMission5Prompt(){ const box=document.getElementById('m5-prompt'); const prog=document.getElementById('m5-progress'); if(!box||!prog) return; const total=missions[5].sequence.length; const current=mission5Current(); if(current){ box.textContent=current.text; prog.textContent=`${state.meta[5].index+1} / ${total}`; setTimeout(()=>speakMission5(),120); } else { box.textContent='Amazing! You finished the camp day.'; prog.textContent=`${total} / ${total}`; } }
function mission5Pick(id){
 const current=mission5Current();
 if(!current){ toast('Mission complete! Click Next.'); return; }
 if(id!==current.id){ playWrong(); toast('Try again. Listen and choose the right picture.'); return; }
 state.placed[5][current.id]=current.id;
 playCorrect();
 toast(`Great! ${current.text}`);
 state.meta[5].index += 1;
 markUsed();
 updateMission5Prompt();
}
...
function reviewSentence(s){const [id,label,,type]=s; if(type==='weather')return `It is <strong>${label}</strong>.`; if(type==='wear')return `I am wearing <strong>${article(label)}</strong>.`; if(type==='camp')return `We are <strong>making camp</strong>.`; if(type==='routine')return `At night, I <strong>go to bed</strong>.`; return `We have got <strong>${article(label)}</strong>.`;}
...
function restoreMission(m){
 ...
 if(m===5){ if(!state.meta[5].order.length) state.meta[5].order=shuffle(missions[5].sources.map(s=>s[0])); updateMission5Prompt(); }
 if(m===6)renderStoryBubbles();
 markUsed();
}
function finish(){
 const shell=document.querySelector('.game-shell');const card=document.createElement('div');card.className='finish-card';card.innerHTML=`<div class="finish-panel"><h2>Camp Quest Complete!</h2><p>You packed, dressed, explored, read, listened and told your own camp story. Great job!</p><button id="playAgain">Play again</button></div>`;shell.appendChild(card);document.getElementById('playAgain').onclick=()=>{for(let i=1;i<=6;i++)state.placed[i]={}; state.meta[2].index=0; state.meta[4]={answers:{},checked:false}; state.meta[5]={index:0,order:shuffle(missions[5].sources.map(s=>s[0]))}; renderStart();};
}
...
function runQA(){
 try{
  ...
  if(state.mission===2){mission2Pick('cap');checks.push(state.meta[2].index===1)}
  ...
  if(state.mission===5){state.meta[5].index=0; mission5Pick('wake-up'); checks.push(state.meta[5].index===1 && !!state.placed[5]['wake-up'])}
...
const startMission=Math.min(6,Math.max(1,Number(new URLSearchParams(location.search).get('m'))||0));
if(startMission>=1){ renderMission(startMission); } else { state.meta[5].order=shuffle(missions[5].sources.map(s=>s[0])); renderStart(); }
})();
