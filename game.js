...
function renderMission(m){
 ...
 if(m===2){overlay+=cfg.sources.map(s=>sourceHtml(m,s)).join('')+cfg.targets.map(t=>targetHtml(t[0],t[1],t[2],'camper-target')).join('')+`<div class="avatar-layer" data-avatar="boy" style="left:32.6%;top:15.8%;width:17.2%;height:53%"></div><div class="avatar-layer" data-avatar="girl" style="left:50.8%;top:15.8%;width:17.2%;height:53%"></div><div class="camper-label" style="left:38.2%;top:67.4%">BOY</div><div class="camper-label" style="left:56.6%;top:67.4%">GIRL</div><div class="hint-banner" style="left:44.8%;top:76.2%;width:12.7%">Drag to BOY or GIRL</div>`;}
 ...
 shell.querySelectorAll('.source').forEach(src=>{
   ...
   src.addEventListener('click',e=>{
     if(e.detail===0||!src.dataset.dragged){
       if(src.classList.contains('used')) toggleUsedSource(src.dataset.source);
...
function removePlacement(id){
 ...
 if(m===2){delete state.placed[2][id]; renderAvatar('boy'); renderAvatar('girl');}
...
function renderWeatherZones(){
 ...
 ['sunny-zone','rainy-zone','snowy-zone'].forEach(targetId=>{
   const tag=document.querySelector(`[data-zone-tag="${targetId}"]`); if(!tag) return; tag.innerHTML='';
   Object.values(p[targetId]||{}).forEach(cardId=>{const im=document.createElement('img');im.src=sourceImg(3,cardId);im.alt=findSource(cardId)[1];im.dataset.item=cardId;tag.appendChild(im)});
   ...
   if(sentence){
     const pos=targetId==='sunny-zone'?'left:8%;top:11%':targetId==='rainy-zone'?'left:40%;top:11%':'left:70%;top:11%';
     sayNear(document.querySelector(`[data-target="${targetId}"]`),sentence,pos);
   }
 })
}
