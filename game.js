...
function mission5CardsHtml(){
 ...
 return ids.map((id,i)=>{
   ...
   return `<button class="source mission5-card" data-source="${id}" aria-label="${src[1]}" title="${src[1]}" style="${pct(cfg.slots[i])}"><span class="mini-check">${icons.check()}</span></button>`;
 }).join('');
}
...
function renderMission(m){
 ...
 if(m===5){overlay+=mission5CardsHtml()+`<div class="mission5-panel"><div class="mission5-title">Listen and choose the picture.</div><div class="mission5-prompt" id="m5-prompt"></div><button class="mission5-listen" data-control="say5">${icons.speech()} Listen again</button><div class="mission5-progress" id="m5-progress"></div></div>`;}
...
function mission5Pick(id){ const current=mission5Current(); if(!current){ toast('Mission complete! Click Next.'); return; } if(id!==current.id){ playWrong(); toast('Try again. Listen and choose the right picture.'); return; } state.placed[5][current.id]=current.id; playCorrect(); toast(`Great! ${current.text}`); state.meta[5].index += 1; state.meta[5].order = shuffle(missions[5].sources.map(s=>s[0])); renderMission(5); }
