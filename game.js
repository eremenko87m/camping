...
const state={mission:1,selected:null,placed:{1:{},2:{},3:{},4:{},5:{},6:{}},storyIndex:0};
...
4:{
 sources:[['tent','tent',rect(45,640,145,155)],['green-sleeping-bag','green sleeping bag',rect(180,640,135,155)],['blue-sleeping-bag','blue sleeping bag',rect(300,640,145,155)],['lantern','lantern',rect(430,640,100,155)],['blue-mug','blue mug',rect(515,640,105,155)],['red-mug','red mug',rect(610,640,110,155)],['sandwiches','sandwiches',rect(700,640,190,155)],['fruit','fruit',rect(880,640,140,155)],['book','book',rect(1010,640,125,155)],['ball','ball',rect(1135,640,115,155)],['teddy','teddy bear',rect(1235,640,115,155)],['backpack','backpack',rect(1335,635,125,160)],['water-bottle','water bottle',rect(1450,640,90,155)],['map','map',rect(1530,640,135,155)]],
 target:{id:'free-stage',label:'camp',rect:rect(25,90,1615,525)}
},
5:{
...
function renderMission(m){
 ...
 if(m===2){overlay+=cfg.sources.map(s=>sourceHtml(m,s)).join('')+cfg.targets.map(t=>targetHtml(t[0],t[1],t[2],'camper-target')).join('')+`<div class="outfit-stack" data-stack="boy" style="left:45.8%;top:49.5%;width:10.8%;height:8.8%"></div><div class="outfit-stack" data-stack="girl" style="left:58.5%;top:49.5%;width:10.8%;height:8.8%"></div>`;}
 if(m===3){overlay+=cfg.sources.map(s=>sourceHtml(m,s)).join('')+cfg.targets.map(t=>targetHtml(t[0],t[1],t[2],'weather-zone')).join('')+`<div class="zone-tag" data-zone-tag="sunny-zone" style="left:3%;top:53%"></div><div class="zone-tag" data-zone-tag="rainy-zone" style="left:36%;top:53%"></div><div class="zone-tag" data-zone-tag="snowy-zone" style="left:68.5%;top:53%"></div>`;}
 if(m===4){overlay+=cfg.sources.map(s=>sourceHtml(m,s)).join('')+targetHtml(cfg.target.id,cfg.target.label,cfg.target.rect,'free-stage');}
...
function bind(){
 ...
 shell.querySelectorAll('.source').forEach(src=>{
   src.addEventListener('pointerdown',e=>startDrag(e,src.dataset.source,false));
   src.addEventListener('click',e=>{if(e.detail===0||!src.dataset.dragged)selectSource(src.dataset.source); delete src.dataset.dragged;});
 });
...
function handleControl(c){
 if(c==='reset'){state.placed[state.mission]={};renderMission(state.mission)}
...
function placeOutfit(id,targetId){
 if(!['boy','girl'].includes(targetId))return; const p=state.placed[2];p[id]={id,target:targetId};
 document.querySelectorAll('.outfit-stack img').forEach(i=>{if(i.dataset.item===id)i.remove()}); const stack=document.querySelector(`[data-stack="${targetId}"]`); const im=document.createElement('img');im.src=sourceImg(2,id);im.dataset.item=id;im.alt=findSource(id)[1];stack.appendChild(im);
 const label=findSource(id)[1], subj=targetId==='boy'?'He':'She'; const has=id.includes('backpack'); const sentence=has?`${subj} has got <strong>${article(label)}</strong>.`:`${subj} is wearing <strong>${article(label)}</strong>.`;
 sayNear(document.querySelector(`[data-target="${targetId}"]`),sentence,targetId==='boy'?'left:36%;top:15%':'left:61%;top:15%');toast(strip(sentence));
}
function placeWeather(id,targetId){
 if(!targetId.endsWith('zone'))return; const type=findSource(id)[3]; const p=state.placed[3];
 // A physical card moves between zones; there is still no correctness judgement.
 Object.keys(p).forEach(zone=>{if(p[zone]?.[type]===id)delete p[zone][type]});
 if(!p[targetId])p[targetId]={}; p[targetId][type]=id;
 const tag=document.querySelector(`[data-zone-tag="${targetId}"]`);tag.innerHTML='';Object.values(p[targetId]).forEach(cardId=>{const im=document.createElement('img');im.src=sourceImg(3,cardId);im.alt=findSource(cardId)[1];tag.appendChild(im)});
 ...
 const pos=targetId==='sunny-zone'?'left:8%;top:11%':targetId==='rainy-zone'?'left:40%;top:11%':'left:70%;top:11%'; if(sentence)sayNear(document.querySelector(`[data-target="${targetId}"]`),sentence,pos);toast(sentence?strip(sentence):'Add another card to build a sentence.');
}
...
function placeStory(id,targetId){
 if(!targetId.startsWith('bubble-'))return;Object.keys(state.placed[6]).forEach(k=>{if(state.placed[6][k]===id)delete state.placed[6][k]});state.placed[6][targetId]=id;const t=document.querySelector(`[data-target="${targetId}"]`);t.querySelectorAll('.placed-thumb,.story-sentence').forEach(e=>e.remove());const im=thumb(id);im.dataset.item=id;im.style.left='3%';t.appendChild(im);const sentence=reviewSentence(findSource(id));const s=document.createElement('div');s.className='story-sentence';s.style.left='49%';s.style.top='18%';s.style.width='48%';s.innerHTML=sentence;t.appendChild(s);toast(strip(sentence));
}
...
function restoreMission(m){
 ...
 if(m===2)Object.values(p).forEach(v=>placeOutfit(v.id,v.target));
 if(m===3)Object.keys(p).forEach(zone=>Object.values(p[zone]).forEach(id=>placeWeather(id,zone)));
 if(m===4)Object.values(p).forEach(v=>renderFreeItem(v.id,v.x,v.y,document.querySelector('[data-target="free-stage"]')));
 if(m===5)renderRoutine();
 if(m===6)Object.keys(p).forEach(t=>placeStory(p[t],t));
 markUsed();
}
function finish(){
 const shell=document.querySelector('.game-shell');const card=document.createElement('div');card.className='finish-card';card.innerHTML=`<div class="finish-panel"><h2>Camp Quest Complete!</h2><p>You packed, dressed, explored, made camp, created your own camp day and told your adventure story.</p><button id="playAgain">Play again</button></div>`;shell.appendChild(card);document.getElementById('playAgain').onclick=()=>{for(let i=1;i<=6;i++)state.placed[i]={};renderMission(1)};
}
...
function runQA(){
 try{
  ...
  if(state.mission===2){place('sun-hat','boy');checks.push(state.placed[2]['sun-hat']?.target==='boy')}
  ...
  if(state.mission===4){place('teddy','free-stage');checks.push(!!state.placed[4].teddy)}
